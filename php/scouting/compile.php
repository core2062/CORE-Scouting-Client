...
function compileScoutingRebuild(){
	global $db;
	global $teams;
	...
	$len = count($teams);
	for($i=0; $i < $len; $i++){
		compileScouting($teams[$i]);
	}
}

function compileScouting($teamNum){
	global $db;

	$obj = $db->sourceTeamInfo->findOne(['_id' => $teamNum]);//start with team info

	unset($obj['meta']);
	unset($obj['events']);//temporary

	//$obj['opr'] = $opr[$teamNum];//add opr data to team object

	//get scouting info
	$cursor = $db->analysisScouting->find(['teamNum' => $teamNum]);
	foreach($cursor as $currentMatch){
		unset($currentMatch['_id']);//just a random id
		unset($currentMatch['teamNum']);

		//error check for duplicate entries on same match
